...
2. **Kiwin Console** - 在 IDE 底部的控制台窗口中打印详细的组装信息，包括：
...
1. 打开任意 Java 文件
2. 将光标定位在方法内部或方法签名上
3. 右键点击，在弹出菜单中选择 **"Kiwin" → "Extract Method Information"**

#### 输出结果

执行该功能后，结果会输出到 **Kiwin Console** 控制台窗口，包括：
...
3. 右键点击，在弹出菜单中选择 **"Kiwin" → "Get Top Callers Information"**
...
Kiwin 采用 DDD（领域驱动设计）分层架构，各层职责清晰，核心业务逻辑与技术实现解耦，便于维护和扩展：
...
构建产物位于：`build/distributions/Kiwin-x.x.x.zip`
...
- 弹窗标题、列标题、Excel Sheet 名称、控制台表格列标题同步更新

### v0.0.77
- 优化顶层调用者控制台输出格式，改为 ASCII 表格形式
- 列顺序与 TreeTable 弹窗保持一致
- 支持中文字符宽度计算

### v0.0.76
- 修复 TreeTable 表头光标样式问题
- 列分隔线区域显示列宽调整光标，可排序列内部区域显示手型光标
...
### v0.0.74
- 顶层调用者 TreeTable 表格新增默认排序规则
- 第一优先级：类型（API 排在 Normal 前面）
- 第二优先级：请求路径（升序）
- 第三优先级：方法全限定名（升序）

### v0.0.73
- 同步更新产品说明书和 README.md 文档
- 补充 SQL 片段顶层调用者查找功能说明
- 补充 TreeTable 层级展示组件介绍
...
**项目仓库**：https://github.com/euver/Kiwin
